<hr class="my-4 opacity-75 container">
<div class="container">
    <div class="row">
        <div class="col mt-5">
            <h3 class="fs-5 display-6">Site par : Thomas Michelin</h3>
            <h3 class="fs-5 display-6">Classe : SIO1</h3>
            <h3 class="fs-5 display-6">Lycée : Louis Pergaud Besançon</h3>

            <a class="button nav-link mt-5 text-primary" href="/index.php">Retour à l'accueil</a>
        </div>
        <div class="col">
            <a class="button nav-link mt-5 text-primary" href="/paiement.php">Politique de paiement</a>
            <a class="button nav-link text-primary" href="/Retour.php">Politique de retour</a>
            <a class="button nav-link text-primary" href="/partenaire.php">Partenaire</a>
        </div>
        <div class="col">
            <img src="./assets/images/LPSGIF.gif" alt="Logo MyDriveMovie" width="300" height="300"
                 class="d-inline-block align-top">
        </div>
    </div>
</div>